      ...
      <section className="pt-[170px]">
        <Container>
          <div className="grid gap-16 lg:grid-cols-[1fr_1.05fr]">
            <div>
              <h1 className="text-[clamp(44px,5vw,61px)] font-semibold leading-none tracking-[-0.07em]">
                Our Services
              </h1>
        ...
        <Container>
          <div className="space-y-10">
            {serviceCards.map((service, index) => (
              <article key={service.slug} className="grid bg-white lg:grid-cols-[410px_1fr]">
                {index % 2 === 1 && (
...
function ServiceImage({ src, title, className = "" }: { src: string; title: string; className?: string }) {
  return (
    <div className={`relative min-h-[360px] overflow-hidden bg-neutral-200 lg:min-h-[460px] ${className}`}>
